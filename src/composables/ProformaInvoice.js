...
const generateProformaIdWithTimestamp = () => {
...
export const downloadProforma = async(invoice) => {
  ...
  const footer = ()=>{
    ...
    doc.text('Proforma Invoice generated on DaqAuto.com ', pageWidth / 2, pageHeight - 10, { align: 'center' })
  }
  ...
  doc.setFont('helvetica', 'bold')
  doc.text('Proforma Invoice', margin+10, yPos)
...
const termsAndConditions = ()=>{
  ...
  if (invoice.notes) {
    ...
    doc.setFontSize(9)
    doc.setFont('helvetica', 'bold')
    doc.text('OTHERS NOTES / CONDITIONS', margin, yPos)
    doc.setFont('helvetica', 'normal')
